...
export async function fetchDashboardData(userId) {
  ...
  function extractTopMetrics(userData) {
    const latestProgress = userData.profile?.progressRecords?.[0] || {};
    const allWorkouts = userData.profile?.workouts || [];
    const allActivities = userData.profile?.activities || [];
    
    return {
      weight: {
        value: userData.personalInfo?.currentWeight || latestProgress.weight || 0,
        unit: "lbs"
      },
      steps: {
        value: allActivities.reduce((sum, a) => sum + (a.steps || 0), 0),
        unit: "steps"
      },
      calories: {
        value: userData.statistics?.caloriesBurned || latestProgress.calories_burnt || 0,
        unit: "kcal"
      }
    };
  }
  ...
  function extractProgressData(userData) {
    const progressRecords = userData.profile?.progressRecords || [];
    const formattedData = [];
    
    // Map progress records into chart-friendly format
    if (progressRecords.length > 0) {
      // Use a Set to track unique months we've processed
      ...
      progressRecords.forEach(record => {
        ...
        if (!processedMonths.has(month)) {
          ...
          // Find matching activities and workouts for this month
          const monthStart = new Date(date.getFullYear(), date.getMonth(), 1);
          const monthEnd = new Date(date.getFullYear(), date.getMonth() + 1, 0);
          
          const monthActivities = (userData.profile?.activities || []).filter(a => {
            const activityDate = new Date(a.date);
            return activityDate >= monthStart && activityDate <= monthEnd;
          });
          
          const monthWorkouts = (userData.profile?.workouts || []).filter(w => {
          ...
          formattedData.push({
            ...
            steps: monthActivities.reduce((sum, a) => sum + (a.steps || 0), 0),
            calories: record.calories_burnt || 0,
            workoutDuration: monthWorkouts.reduce((sum, w) => sum + (w.duration || 0), 0)
          });
        }
      });
    }
    
    // If no progress data, return default
  ...
  function extractWeeklyActivity(userData) {
    ...
    const recentActivities = (userData.profile?.activities || []).filter(a => {
      const date = new Date(a.date);
      return date >= weekAgo && date <= today;
    });
    
    // Add recent workout minutes to the appropriate day
  ...
  function extractWorkoutTypes(userData) {
    const typeCount = {};
    const workouts = userData.profile?.workouts || [];
  ...
  function generateInsights(userData) {
    ...
    (userData.profile?.workouts || []).forEach(w => {
